  ...
  */
  DYNAMIC.prototype = {
    ...
    setVertexVertexNormals: function(){

      for(var i = 0; i < this.target.faces.length; i++){
        
        var face = this.target.faces[i];
        ...
        for(var j = 0; j < face.vertexNormals.length; j++){
          ...
          if (j == 0){
            vert = this.target.vertices[face.a];
          }else if (j == 1){
            vert = this.target.vertices[face.b];
          }else if (j == 2){
            vert = this.target.vertices[face.c];
          }else if (j == 3){
            vert = this.target.vertices[face.d];
          }
      ...
      this.target.facesNeedUpdate = true;
      this.target.normalsNeedUpdate = true;
    ...
    returnToRegularNormal:function(){
  
      this.target.computeVertexNormals();
      this.setVertexVertexNormals();
    ...
    returnToOG: function(){
      ...
      //TODO
      //Make this function work so that the vertices will return to 
      //the OG vertices, without destroying the normals
